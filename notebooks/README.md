...
The pre-commit hook uses **AST parsing** (not regex) to detect banned imports like `sklearn.metrics` and `sklearn.calibration` -- catching all import forms including `from sklearn import metrics` and aliased imports. Sensitive data patterns (patient IDs, absolute home paths) are scanned in code cells only.
