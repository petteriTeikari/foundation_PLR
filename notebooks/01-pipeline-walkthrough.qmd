...
```{mermaid}
%%| label: fig-pipeline
%%| fig-cap: "Four-stage preprocessing-to-classification pipeline."
flowchart LR
...
```{mermaid}
%%| label: fig-stimulus-protocol
%%| fig-cap: "Chromatic pupillometry stimulus protocol."
sequenceDiagram
...
# Table names from SHOW TABLES are trusted internal values (not user input)
...
```{python}
#| label: method-counts
# Column names below are hardcoded constants (not user input)
...
from src.viz.plot_config import setup_style, COLORS
...
ax.hist(auroc_values["auroc"], bins=30, edgecolor=COLORS["grid"], linewidth=0.5)
ax.set_xlabel("AUROC")
...
```{mermaid}
%%| label: fig-stratos-domains
%%| fig-cap: "Five STRATOS evaluation domains."
flowchart TB
