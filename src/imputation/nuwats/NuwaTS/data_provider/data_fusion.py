...
from src.imputation.nuwats.NuwaTS.data_provider.allinone_dataset import (
    ForecastGrid as all_in_one_dataset,
)
import time
import os
...
def load_dataset_dataloader(
    ...
    for i in range(len(paths)):
        ...
        if flag == "train":
            dataset = all_in_one_dataset(
                data_load[:, :num_sensors_per_set],
                window_size=input_length,
                horizon=horizon,
                normalize_method="z_score",
                norm_statistic=None,
                interval=1,
                origin_missrate=origin_missrate,
            )
        elif flag == "val":
            dataset = all_in_one_dataset(
                data_load[:, num_sensors_per_set : 2 * num_sensors_per_set],
                window_size=input_length,
                horizon=horizon,
                normalize_method="z_score",
                norm_statistic=None,
            ...
            dataset = all_in_one_dataset(
                data_load[:, 2 * num_sensors_per_set : 3 * num_sensors_per_set],
                window_size=input_length,
                horizon=horizon,
                normalize_method="z_score",
                norm_statistic=None,
...
if __name__ == "__main__":
    ...
    _, train_loader = load_dataset_dataloader(
        input_length=96,
        horizon=96,
        batch_size=128,
        num_workers=8,
