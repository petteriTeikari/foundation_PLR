...
from src.imputation.nuwats.NuwaTS.layers.Embed import DataEmbedding
...
class PrefixEncoder(torch.nn.Module):
    ...
    def forward(self, prefix, knowledge_embeddings=None):
        ...
        if self.prefix_projection:
            ...
            if knowledge_embeddings != None:  # noqa: E711
                knowledge_embeddings = knowledge_embeddings.repeat(
                    past_key_values.size(0), 1, 1
                )
                knowledge_past_key_values = self.knowledge_trans(knowledge_embeddings)
                past_key_values = (
                    past_key_values + knowledge_past_key_values * self.alpha
                )
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.patch_num = max(
            self.seq_len // self.patch_size, self.pred_len // self.patch_size
        )
        ...
        if not self.configs.use_llama:
            ...
            if self.configs.use_bert:
                ...
                # https://github.com/Chengyui/NuwaTS/issues/2#issuecomment-2481267717
                self.gpt2_config = GPT2Config()
                self.gpt2 = GPT2Model(self.gpt2_config)
        ...
        self.contrastive_patch_projector = nn.Linear(
            configs.d_model, self.contrastive_dim
        )
        self.contrastive_instance_projector = nn.Linear(
            configs.d_model * self.patch_num, self.contrastive_dim
        )
        self.instance_W = nn.Parameter(
            torch.rand(self.contrastive_dim, self.contrastive_dim)
        )
        ...
        if self.is_prefix_tuning:
            self.prefix_tokens = nn.Parameter(
                torch.rand(self.prefix_length, configs.d_model)
            )
        ...
        if self.is_prefix_tuningv2:
            ...
            self.prefix_encoder = PrefixEncoder(
                pre_seq_len=configs.prefix_length,
                hidden_size=configs.d_model,
                prefix_hidden_size=configs.d_model,
                num_hidden_layers=configs.gpt_layers,
                prefix_projection=True,
            )

        if self.configs.continue_tuning:
            self.prefix_continue_token = nn.Parameter(
                torch.rand(self.prefix_length, configs.d_model)
            )
        ...
        if self.configs.continue_tuningv2:
            ...
            self.prefix_continue_encoder = PrefixEncoder(
                pre_seq_len=configs.prefix_length,
                hidden_size=configs.d_model,
                prefix_hidden_size=configs.d_model,
                num_hidden_layers=configs.gpt_layers,
        ...
        if not self.is_seq_output:
            ...
            self.seq_token = nn.Parameter(
                torch.rand(self.configs.seq_token, self.configs.d_model)
            )
            self.out_layer = nn.Linear(
        ...
        if self.configs.alignment:
            self.reprogramming_layer = ReprogrammingLayer(
                self.configs.d_model, 8, None, self.configs.d_model
            )
    ...
    def get_prompt(self, batch_size, variable_num, knowledge_embeddings=None):
        ...
        prefix_tokens = (
            ...
            .expand(batch_size * variable_num, -1)
            .to(self.device)
        )
        if knowledge_embeddings is not None:
            past_key_values = self.prefix_continue_encoder(
                prefix_tokens, knowledge_embeddings=knowledge_embeddings
            )
        else:
            past_key_values = self.prefix_encoder(prefix_tokens)
        ...
        past_key_values = past_key_values.view(
            bsz, seqlen, self.configs.gpt_layers * 2, 12, self.configs.d_model // 12
        )
    ...
    def forward(
        self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None, skip_output=False
    ):
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask, skip_output=False):
        if mask is None:
            ...
            print(
                "add check for nan"
            )  # https://github.com/Chengyui/NuwaTS/issues/2#issuecomment-2490794728
        ...
        if self.configs.cov_prompt:
            x_enc_seg = rearrange(
                x_enc,
                "b (patch_num patch_size) n -> (b patch_num) patch_size n",
            ...
            patch_feature = torch.cat(
                [
                    patch_min_values.unsqueeze(-1),
                    patch_medians.unsqueeze(-1),
                    patch_max_values.unsqueeze(-1),
                    patch_trends.unsqueeze(-1),
                ],
                dim=-1,
            )
            variable_feature = torch.cat(
                [
                    variable_min_values.unsqueeze(-1),
                    variable_medians.unsqueeze(-1),
                    variable_max_values.unsqueeze(-1),
                    variable_trends.unsqueeze(-1),
                ],
                dim=-1,
            )
            patch_feature = rearrange(
                patch_feature,
                "(b patch_num) n feature_num -> (b n) patch_num feature_num",
                patch_num=patch_num,
            )
            variable_feature = rearrange(
                variable_feature, "b n feature_num -> (b n) feature_num"
            ).unsqueeze(1)
            ...
            missing_embedding = torch.matmul(
                patch_missing_rate.unsqueeze(-1), self.miss_token
            )  # (bn)
            missing_embedding = rearrange(
                missing_embedding,
                "(b patch_num) n d_model -> (b n) patch_num d_model",
                patch_num=patch_num,
            )

        x_enc = rearrange(
            x_enc,
            "b (patch_num patch_size) c -> (b c) patch_num patch_size",
            patch_size=self.patch_size,
        )
        ...
        if self.is_prefix_tuning:
            if self.configs.continue_tuning:
                ...
                prefix_tokens = repeat(
                    prefix_tokens,
                    "seq_len dmodel -> repeat seq_len dmodel",
        ...
        if self.configs.continue_tuningv2:
            ...
            outputs = self.gpt2(
                inputs_embeds=gpt2_enc_out, past_key_values=past_key_values
            ).last_hidden_state
        else:
            if self.is_prefix_tuningv2:
                if self.configs.continue_tuningv2:
                    past_key_values = self.get_prompt(
                        batch_size=B,
                        variable_num=N,
                        knowledge_embeddings=self.prefix_encoder.embedding(
                            torch.tensor(0).to(self.gpt2.device)
                        ),
                    )
                else:
