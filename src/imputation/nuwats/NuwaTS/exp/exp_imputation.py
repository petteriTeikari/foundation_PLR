from src.imputation.nuwats.NuwaTS.data_provider.data_factory import data_provider
from src.imputation.nuwats.NuwaTS.exp.exp_basic import Exp_Basic
from src.imputation.nuwats.NuwaTS.utils.tools import (
    EarlyStopping,
    adjust_learning_rate,
    visual,
)
from src.imputation.nuwats.NuwaTS.utils.metrics import metric
...
class Exp_Imputation(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                batch_ind = (
                    torch.arange(B, device=self.device).unsqueeze(-1).unsqueeze(-1)
                )
                sensor_ind = (
                    torch.arange(N, device=self.device).unsqueeze(0).unsqueeze(0)
                )
    ...
    def train(self, setting):
        ...
        if (
            self.args.prefix_tuningv2
            or self.args.prefix_tuning
            or self.args.continue_tuningv2
            or self.args.continue_tuning
        ):
        ...
        if self.args.origin_missrate > 0:
            print(
                "Training With dataset origin missrate:{}".format(
                    self.args.origin_missrate
                )
            )
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                inp = batch_x.masked_fill(mask == 0, 0)
                outputs, representation = self.model(
                ...
                if (
                    self.args.patch_con
                    or self.args.temporal_con
                    or self.args.flatten_con
                ):
                    con_loss, con_output = self.contrastive_loss(
                        batch_x, batch_x_mark, representation
                    )
                    loss = (
                        criterion(outputs, batch_x)
                        + criterion(con_output, batch_x)
                        + con_loss * self.args.con_weight
                    )
                else:
                ...
                if (i + 1) % 500 == 0:
                    print(
                        "\titers: {0}, epoch: {1} | loss: {2:.7f} con loss : {3:.7f}".format(
                            i + 1, epoch + 1, loss.item(), con_loss.item()
                        )
                    )
            ...
            print(
                "Adam lr epoch:{} lr:{}".format(
                    epoch, model_optim.param_groups[0]["lr"]
                )
            )
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                    ...
                    if i % 200 == 0:
                        ...
                        visual(
                            ...
                            filled_pred,
                            os.path.join(
                                folder_path, str(i) + "rate{}.pdf".format(mask_rate)
                            ),
                            mask_rate=mask_rate,
                        )
                        visual(
                            ...
                            os.path.join(
                                folder_path,
                                str(i) + "origin_rate{}.pdf".format(mask_rate),
                            ),
                            mask_rate=mask_rate,
                        )
    ...
    def contrastive_loss(self, batch_x, batch_x_mark, representation):
        ...
        con_loss = self.infoNCE(
            outputs1,
            outputs2,
            is_patch=self.args.patch_con,
            is_temporal_patch=self.args.temporal_con,
            flatten_con=self.args.flatten_con,
        )
    ...
    def infoNCE(
        ...
        if is_patch:
            ...
            patch_W = repeat(
                self.model.patch_W, "dim1 dim2 -> repeat dim1 dim2", repeat=patch_num
            )
        ...
        if is_temporal_patch:
            ...
            patch_W = repeat(
                self.model.patch_W, "dim1 dim2 -> repeat dim1 dim2", repeat=batch_size
            )
        ...
        if flatten_con:
            ...
            z_a_f = rearrange(
                z_a_f,
                "(b con_num) dmodel -> b con_num dmodel",
                con_num=best_contrastive_num,
            )
            z_pos_f = rearrange(
                z_pos_f,
                "(b con_num) dmodel -> b con_num dmodel",
            ...
            patch_W = repeat(
                self.model.patch_W,
                "dim1 dim2 -> repeat dim1 dim2",
                repeat=flatten_batch,
            )
