...
from src.utils import get_artifacts_dir, pandas_concat, pandas_col_condition_filter


def if_refeaturize(cfg):
...
def combine_df_with_outputs(df, data_dict, imputation, split, split_key, model_name):
...
def combine_standardized_inputs_with_df(df, data_dict):
...
def combine_inputation_with_df(df, imputation):
...
def subjects_with_class_labels(df, split):
...
def pick_correct_split(
    data_dict, split, split_key, eval_results, model_name, standardize_stats
):
...
def pick_input_data(input_data, split, split_key, model_name):
    """
    :param input_data: dict containing the preprocessed data
    :param split: e.g. "train"
    :param split_key: e.g. "train_gt" or "train_raw"
    :param model_name: e.g. "CDSI"
    """
    if "raw" in split_key:
...
def get_light_stimuli_timings(df_subject):
    def set_null_to_zero(df, col):
        df = df.with_columns(
            (
    ...
    def individual_light_timing(df_subject, color: str = "Red"):
...
def get_top1_of_col(df, col: str, descending: bool):
...
def replace_zeros_with_null(df, col):
...
def get_feature_samples(
    df_subject, feature_params, col="time", feature: str = None
) -> pl.DataFrame:
    """
    Estoeric issue possible here with Polars library, some Rust error that does
    not raise a Python exception, and can be due to Object numpy arrays, negative time
    or something not so intuitive

    pyo3_runtime.PanicException: implementation error" possible
    https://github.com/pola-rs/polars/issues/18399
    Problem with some column being an Object class?
    feature_samples = df_subject.filter(
            (pl.col(col) >= feature_params["time_start"])
            & (pl.col(col) <= feature_params["time_end"])
        )
    """
...
def flatten_dict_to_dataframe(features_nested, mlflow_series: pl.Series, cfg):
...
def flatten_subject_dicts_to_df(subjects_as_dicts: dict, cfg: DictConfig):
...
def create_df_row(subject_dict, subject_code, cfg):
...
def get_features_fpath(cfg, service_name: str = "best_models"):
    filename, ext = os.path.splitext(cfg["DATA"]["filename_DuckDB"])
    filename += (
        cfg["PLR_FEATURIZATION"]["feature_file_suffix"]
        + "."
        + cfg["ARTIFACTS"]["results_format"]
    )
    artifacts_dir = get_artifacts_dir(service_name=service_name)
    return os.path.join(artifacts_dir, filename)


def export_features_to_disk(dict_out, cfg):
    features_filepath = get_features_fpath(cfg)
    basedir, fname = os.path.split(features_filepath)
    os.makedirs(basedir, exist_ok=True)

    logger.info("Exporting the features to the disk: {}".format(features_filepath))
    save_results_dict(dict_out, features_filepath, debug_load=True)


def load_features_from_disk(cfg):
    features_filepath = get_features_fpath(cfg)
...
def get_feature_names(
    features, cols_exclude=("subject_code",), name_substring="_value"
):
...
def get_split_keys(features, model_exclude="BASELINE_GT", return_suffix=True):
...
def data_for_featurization_wrapper(artifacts: dict, cfg: DictConfig):
...
def get_baseline_input_data_for_featurization(
    artifacts: dict, model_name: str, split_names: list
):
...
def get_imputed_data_for_featurization(artifacts: dict, cfg: DictConfig):
...
def imputed_data_by_split_key(imputation, metadata, split, split_key) -> dict:
    imputed_data = {"data": imputation, "metadata": metadata}
    return imputed_data


def get_pseudoimputation_dicts_from_input_data(model_artifacts, split):
...
def get_dict_PLR_per_code(data_dict, i):
    dict_PLR = {}
    dict_tmp = data_dict
...
def drop_useless_metadata_cols(metadata_subject, i, cfg):
...
def subjectdict_to_df(dict_PLR):
...
def get_df_subject_per_code(data_df, subject_code, cfg):
...
def export_features_pickle_file(features: dict, data_source: str, cfg: DictConfig):
    ...
    features: dict
      data: dict
        - train: pl.DataFrame
        - test: pl.DataFrame (no_subjects, no_feature-like columns), e.g. (16, 25)
      mlflow_run: ?
    """
    output_dir = get_artifacts_dir(service_name="features")
    os.makedirs(output_dir, exist_ok=True)
    fname = get_features_pickle_fname(data_source)
    output_path = os.path.join(output_dir, fname)
    try:
        save_results_dict(features, output_path, name="features")
    except Exception as e:
...
def harmonize_to_imputation_dict(
    ...
    """
    Output must now conform to the imputation dictionary format (model_dict['imputation'][split])
    """
...
def get_original_data_per_split_key(model_dict: dict, cfg: DictConfig, split_key: str):
    results_out = {}
...
def name_imputation_sources_for_featurization(sources: list, cfg: DictConfig):
    sources_features = []
...
def get_original_data_to_results(model_dict, cfg):
    results = {}
...
def get_imputed_results(model_dict, cfg):
    results_out = {
...
def check_and_fix_df_schema(df_subject):
