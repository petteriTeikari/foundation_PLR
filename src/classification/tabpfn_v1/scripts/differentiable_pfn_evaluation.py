...
def eval_model(
    ...
    add_name,
    base_path,
    eval_positions_valid=[1000],
    eval_positions_test=[1000],
    bptt_valid=2000,
    bptt_test=2000,
    device="cpu",
    eval_addition="",
    ...
    **extra_tuning_args,
    ...
    model, c, results_file = load_model_workflow(
        i, e, add_name, base_path, device, eval_addition
    )
    params = {
        "bptt": bptt_valid,
    ...
    metrics, metrics_valid, style, temperature, optimization_route = (
        evaluate_point_model(model, **params, **extra_tuning_args)
    )
    ...
    r = [
        ...
        metrics,
        metrics_valid,
        style.to("cpu") if style else style,
        temperature.to("cpu") if temperature else temperature,
        optimization_route,
    ]
            ...
            for split in final_splits:
                splits += [
                    eval_step(
                        test_datasets,
                        None,
                        softmax_temperature=torch.tensor([0]),
                        return_tensor=False,
                        eval_positions=eval_positions_test,
                        bptt=bptt_final,
                        split_number=split,
                        model=model[2],
                        device=device,
...
def eval_step(
    ...
    **kwargs,
    ...
    calculate_score_per_method(
        selection_metric, "select", r, ds, eval_positions, aggregator="mean"
    )
    calculate_score_per_method(
        evaluation_metric, "eval", r, ds, eval_positions, aggregator="mean"
    )

    return r


def gradient_optimize_style(
    model,
    init_style,
    steps,
    softmax_temperature,
    ...
    optimize_all=False,
    limit_style=True,
    N_datasets_sampled=90,
    optimize_softmax_temperature=True,
    ...
    **kwargs,
    ...
    best_style, best_temperature, best_selection_metric, best_diffable_metric = (
        grad_style.detach(),
        softmax_temperature.detach(),
        None,
        None,
    )
    softmax_temperature = torch.nn.Parameter(
        softmax_temperature.detach(), requires_grad=optimize_softmax_temperature
    )
    variables_to_optimize = (
        model[2].parameters() if optimize_all else [grad_style, softmax_temperature]
    )
    ...
    def eval_all_datasets(datasets, propagate=True):
        ...
        for ds in datasets:
            diffable_metric_train, selection_metric_train = eval_opt(
                [ds], inference_mode=(not propagate)
            )
            if not torch.isnan(diffable_metric_train).any():
                if propagate and diffable_metric_train.requires_grad == True:  # noqa E712
                    diffable_metric_train.backward()
                selection_metrics_this_step += [selection_metric_train]
                diffable_metrics_this_step += [
                    float(diffable_metric_train.detach().cpu().numpy())
                ]
    ...
    for t in tqdm(range(steps), desc="Iterate over Optimization steps"):
        ...
        is_best = is_best or (
            ...
            and best_selection_metric > selection_metric_valid
        )
        is_best = is_best or (
            selection_metric_min_max == "max"
            and best_selection_metric < selection_metric_valid
        ...
        if not np.isnan(selection_metric_valid) and is_best:
            ...
            best_selection_metric, best_diffable_metric = (  # noqa F841
                selection_metric_valid,
                diffable_metric_valid,
            )
        ...
        print(
            f"Valid: Diffable metric={diffable_metric_valid} Selection metric={selection_metric_valid};"
    ...
    return {
        ...
        "optimization_route": {
            "select": optimization_route_selection,
            "loss": optimization_route_diffable,
            "test_select": optimization_route_selection_valid,
            "test_loss": optimization_route_diffable_valid,
        },
    }
