...
def eval_model(
    ...
    **kwargs,
    ...
    metrics_test, config_sample, model_path = eval_model_on_ds(
        i,
        e,
        test_datasets,
        eval_positions,
    ...
    metrics_valid, _, _ = eval_model_on_ds(
        i,
        e,
        valid_datasets,
        eval_positions,
    ...
    return {
        "mean_auc_test": metrics_test["mean_roc_at_1000"],
        "mean_auc_valid": metrics_valid["mean_roc_at_1000"],
        "mean_ce_test": metrics_test["mean_ce_at_1000"],
        "mean_ce_valid": metrics_valid["mean_ce_at_1000"],
        "config_sample": config_sample,
        "model_path": model_path,
    }


def eval_model_on_ds(
    i,
    e,
    valid_datasets,
    eval_positions,
    ...
    **kwargs,
    ...
    model, config_sample = load_model(  # noqa F405
        base_path, model_file, device, None, verbose=False
    )
    ...
    for [ds_name, X, y, categorical_feats, _, _] in datasets:
        ...
        for eval_position in eval_positions if verbose else eval_positions:
            eval_position_real = (
                ...
                if 2 * eval_position > dataset_bptt
                else eval_position
            )
            ...
            if isinstance(model, nn.Module) and "BarDistribution" in str(
                type(model.criterion)
            ):
                ys = (ys - torch.min(ys, axis=0)[0]) / (
                    torch.max(ys, axis=0)[0] - torch.min(ys, axis=0)[0]
                )
            ...
            if (
                ...
                and "BarDistribution" in str(type(model.criterion))
                and (
                    metric_used == tabular_metrics.r2_metric
                    or metric_used == tabular_metrics.root_mean_squared_error_metric
                )
            ):
            ...
            new_metric = torch_nanmean(
                torch.stack(
                    [metric_used(ys[i], outputs[i]) for i in range(ys.shape[0])]
                )
            )
        ...
        if num > 0:
            aggregated_metric_datasets, num_datasets = (
                (aggregated_metric_datasets + (aggregated_metric / num)),
                num_datasets + 1,
            )
...
def generate_valid_split(X, y, bptt, eval_position, is_classification, split_number=1):
    ...
    perm = (
        torch.randperm(X.shape[0]) if split_number > 1 else torch.arange(0, X.shape[0])
    )
    X, y = X[perm], y[perm]
    while not done:
        if seed > 20:
        ...
        if is_classification:
            ...
            # classes) and
                ...
                torch.unique(y_[:eval_position]) == torch.unique(y_[eval_position:])
            )
            seed = seed + 1
        else:
            done = True
...
def evaluate_position(
    ...
    **kwargs,
    ...
    if save:
        path = os.path.join(
            base_path,
            f"results/tabular/{path_interfix}/results_{method}_{ds_name}_{eval_position}_{bptt}_{split_number}.npy",
        )
        # log_path =
    ...
    if isinstance(
        model, nn.Module
    ):  # Two separate predict interfaces for transformer and baselines
        outputs, best_configs = (
            transformer_predict(  # noqa F405
                model,
                eval_xs,
                eval_ys,
                eval_position,
        ...
        _, outputs, best_configs = baseline_predict(  # noqa F405
            model,
            eval_xs,
            eval_ys,
            categorical_feats,
