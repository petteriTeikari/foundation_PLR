...
def is_classification(metric_used):
    if (
        metric_used.__name__ == tabular_metrics.auc_metric.__name__
        or metric_used.__name__ == tabular_metrics.cross_entropy.__name__
    ):
...
def get_scoring_string(metric_used, multiclass=True, usage="sklearn_cv"):
    if metric_used.__name__ == tabular_metrics.auc_metric.__name__:
        ...
        if usage == "autosklearn":
            ...
            usage == "catboost"
        ):  # Catboost cannot directly optimize r2 ("Can't be used for optimization." - docu)
            return "RMSE"
        else:
            return "r2"
    elif (
        metric_used.__name__ == tabular_metrics.root_mean_squared_error_metric.__name__
    ):
        if usage == "autosklearn":
            import autosklearn.classification
...
def eval_f(params, clf_, x, y, metric_used):
    scores = cross_val_score(
        ...
        y,
        cv=CV,
        scoring=get_scoring_string(metric_used, usage="sklearn_cv"),
    )
    if (
        get_scoring_string(metric_used, usage="sklearn_cv") == "r2"
        or get_scoring_string(metric_used, usage="sklearn_cv") == "neg_log_loss"
    ):
...
def get_incumbent_results(run_history_file: str, search_space):
    """
    ...
    sorted_runvalue_by_cost = sorted(
        run_history_data.items(), key=lambda item: item[1].cost
    )
...
def well_tuned_simple_nets_metric(
    X_train,
    y_train,
    X_test,
    y_test,
    ...
    categorical_indicator = np.array(
        [i in categorical_indicator for i in range(X_train.shape[1])]
    )
    with tempfile.TemporaryDirectory(
        ...
        def safe_int(x):
            assert np.all(x.astype("int64") == x) or np.any(x != x), np.unique(
                x
            )  # second condition for ignoring nans
            return pd.Series(x, dtype="category")

        X_train = pd.DataFrame(
            {
                i: safe_int(X_train[:, i]) if c else X_train[:, i]
                for i, c in enumerate(categorical_indicator)
            }
        )
        X_test = pd.DataFrame(
            {
                i: safe_int(X_test[:, i]) if c else X_test[:, i]
                for i, c in enumerate(categorical_indicator)
            }
        )
        ...
        pipeline_update, search_space_updates, include_updates = (
        ...
        if number_of_configurations_limit != 0:
            configuration, incumbent_run_value = get_incumbent_results(
                os.path.join(
                    temp_dir, "smac3-output", "run_{}".format(seed), "runhistory.json"
                ),
                search_space,
            )
        ...
        train_predictions = fitted_pipeline.predict(X_train)  # noqa F841
...
def autosklearn_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300):
    return autosklearn2_metric(
        x, y, test_x, test_y, cat_features, metric_used, max_time=max_time, version=1
    )
...
def autosklearn2_metric(
    ...
    clf = clf_(
        ...
        metric=get_scoring_string(
            metric_used, usage="autosklearn", multiclass=len(np.unique(y)) > 2
        ),
    )
...
param_grid_hyperopt["ridge"] = {
    ...
    "alpha": hp.loguniform("alpha", -5, math.log(5.0)),
}  # 'normalize': [False],
...
def lightautoml_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300):
    ...
    oof_pred = automl.fit_predict(tr_data, roles=roles)  # noqa F841
    ...
    if class_map:
        ...
        for i in range(0, len(column_to_class)):
            probabilities_mapped[:, int(column_to_class[int(i)])] = probabilities[
                :, int(i)
            ]
...
def lightgbm_metric(
    x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None
):
    x, y, test_x, test_y = preprocess_impute(
        x,
        y,
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "lightgbm", clf_, metric_used, max_time, no_tune
    )
...
param_grid_hyperopt["logistic"] = {
    ...
    "max_iter": hp.randint("max_iter", 50, 500),
    "fit_intercept": hp.choice("fit_intercept", [True, False]),
    "C": hp.loguniform("C", -5, math.log(5.0)),
}  # 'normalize': [False],
...
def logistic_metric(
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "logistic", clf_, metric_used, max_time, no_tune
    )
...
param_grid_hyperopt["random_forest"] = {
...
def random_forest_metric(
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "random_forest", clf_, metric_used, max_time, no_tune
    )
...
def gradient_boosting_metric(
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "gradient_boosting", clf_, metric_used, max_time, no_tune
    )


## SVM
param_grid_hyperopt["svm"] = {
...
def svm_metric(
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "svm", clf_, metric_used, max_time, no_tune
    )
...
def knn_metric(
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "knn", clf_, metric_used, max_time, no_tune
    )
...
param_grid_hyperopt["gp"] = {
    ...
    "params_length_scale": hp.loguniform(
        "params_length_scale", math.log(0.1), math.log(1.0)
    ),
}
...
def gp_metric(
    ...
    def clf_(params_y_scale=None, params_length_scale=None, **params):
        kernel = (
            params_y_scale * RBF(params_length_scale)
            if params_length_scale is not None
            else None
        )
        if is_classification(metric_used):
            return GaussianProcessClassifier(kernel=kernel, **params)
        else:
            return GaussianProcessRegressor(kernel=kernel, **params)  # noqa F821

    return eval_complete_f(
        x, y, test_x, test_y, "gp", clf_, metric_used, max_time, no_tune
    )
...
def tabnet_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300):
    ...
    x, y, test_x, test_y = preprocess_impute(
        x, y, test_x, test_y, impute=True, one_hot=False, standardize=False
    )
    ...
    best = fmin(
        fn=lambda params: tabnet_eval_f(
            params, clf_, x, y, metric_used, start_time, max_time
        ),
    ...
    return metric, pred, params_used[best_idx]  # noqa F821
...
def catboost_metric(
    ...
    x[:, cat_features], test_x[:, cat_features] = (
        np.nan_to_num(x[:, cat_features], -1),
        np.nan_to_num(test_x[:, cat_features], -1),
    )
    ...
    def clf_(**params):
        if is_classification(metric_used):
            ...
            return CatBoostRegressor(  # noqa F821
                loss_function=get_scoring_string(metric_used, usage="catboost"),
                thread_count=MULTITHREAD,
                used_ram_limit="4gb",
                random_seed=int(y[:].sum()),
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "catboost", clf_, metric_used, max_time, no_tune
    )
...
def xgb_metric(
    ...
    return eval_complete_f(
        x, y, test_x, test_y, "xgb", clf_, metric_used, max_time, no_tune
    )
...
## Ridge
from sklearn.linear_model import RidgeClassifier  # noqa F402

param_grid["ridge"] = {
    "alpha": [0, 0.1, 0.5, 1.0, 2.0],
...
def mlp_acc(x, y, test_x, test_y, hyperparameters):
    ...
    x, y = x.to(device), y.to(device)  # noqa F821
    test_x, test_y = test_x.to(device), test_y.to(device)  # noqa F821
    ...
    def get_model():
        model = nn.Sequential(
            *[
                ...
                for module in [
                    nn.Linear(
                        hidden_dim if layer_idx > 0 else num_features,
                    ...
                    torch.nn.Identity()
                    if layer_idx == num_layers - 1
                    else torch.nn.Dropout(p=fixed_dropout_prob, inplace=False),
                ]
            ]
        )
        if is_binary_classification:
            model.add_module(
                str(len(model)), torch.nn.Softmax(dim=1)
            )  # TODO might also just do an round!?
        return model

    model = get_model().to(device)  # noqa F821
