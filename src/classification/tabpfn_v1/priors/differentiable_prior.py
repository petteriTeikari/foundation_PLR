...
class DifferentiableHyperparameter(nn.Module):
    ...
    def __init__(self, distribution, embedding_dim, device, **args):
        ...
        def get_sampler():
            ...
            if self.distribution == "uniform":
                if not hasattr(self, "sample"):
                    return (
                        uniform_sampler_f(self.min, self.max),
                        self.min,
                        self.max,
                        (self.max + self.min) / 2,
                        math.sqrt(
                ...
                return (
                    uniform_int_sampler_f(self.min, self.max),
                    self.min,
                    self.max,
                    (self.max + self.min) / 2,
                    math.sqrt(1 / 12 * (self.max - self.min) * (self.max - self.min)),
                )
        ...
        if self.distribution.startswith("meta"):
            ...
            def sample_meta(f):
                indicators, passed = unpack_dict_of_tuples(
                    {hp: self.hparams[hp]() for hp in self.hparams}
                )
            ...
            if self.distribution == "meta_beta":
                ...
                if hasattr(self, "b") and hasattr(self, "k"):
                    self.hparams = {
                        "b": lambda: (None, self.b),
                        "k": lambda: (None, self.k),
                    }
                else:
                    self.hparams = {
                        "b": DifferentiableHyperparameter(
                            distribution="uniform",
                            min=self.min,
                            max=self.max,
                            **args_passed,
                        ),
                        "k": DifferentiableHyperparameter(
                            distribution="uniform",
                            min=self.min,
                            max=self.max,
                            **args_passed,
            ...
            if self.distribution == "meta_gamma":
                ...
                if hasattr(self, "alpha") and hasattr(self, "scale"):
                    self.hparams = {
                        "alpha": lambda: (None, self.alpha),
                        "scale": lambda: (None, self.scale),
                    }
                else:
                    self.hparams = {
                        "alpha": DifferentiableHyperparameter(
                            distribution="uniform",
                            min=0.0,
                            max=math.log(self.max_alpha),
                            **args_passed,
                        ),
                        "scale": DifferentiableHyperparameter(
                            distribution="uniform",
                            min=0.0,
                            max=self.max_scale,
                            **args_passed,
                ...
                if not hasattr(self, "log_mean"):
                    self.hparams = {
                        "log_mean": DifferentiableHyperparameter(
                            distribution="uniform",
                            min=math.log(self.min_mean),
                            max=math.log(self.max_mean),
                            **args_passed,
                        ),
                        "log_std": DifferentiableHyperparameter(
                            distribution="uniform",
                            min=math.log(self.min_std),
                            max=math.log(self.max_std),
                            **args_passed,
                        ),
                    }
                else:
                ...
                self.sampler = lambda make_trunc_norm=make_trunc_norm: sample_meta(
                    make_trunc_norm
                )
                ...
                self.max_std = self.max_std if hasattr(self, "max_std") else 1.0
                self.hparams = {
                    "mean": DifferentiableHyperparameter(
                        distribution="uniform",
                        min=self.min_mean,
                        max=self.max_mean,
                        **args_passed,
                    ),
                    "std": DifferentiableHyperparameter(
                        distribution="uniform",
                        min=self.min_std,
                        max=self.max_std,
                        **args_passed,
                ...
                if hasattr(self, "choice_1_weight"):
                    self.hparams = {
                        f"choice_{i}_weight": lambda: (
                            None,
                            getattr(self, f"choice_{i}_weight"),
                        )
                        for i in range(1, len(self.choice_values))
                    }
                else:
                    self.hparams = {
                        f"choice_{i}_weight": DifferentiableHyperparameter(
                            distribution="uniform", min=-3.0, max=5.0, **args_passed
                        )
                        for i in range(1, len(self.choice_values))
                    }
                ...
                def make_choice(**choices):
                    choices = torch.tensor(
                        [1.0] + [choices[i] for i in choices], dtype=torch.float
                    )
                ...
                if hasattr(self, "choice_1_weight"):
                    ...
                    self.hparams = {
                        f"choice_{i}_weight": DifferentiableHyperparameter(
                            distribution="uniform", min=-5.0, max=6.0, **args_passed
                        )
                        for i in range(1, len(self.choice_values))
                    }
            ...
            self.sampler = lambda: return_two(
                self.sampler_f(),
                min=self.sampler_min,
                max=self.sampler_max,
...
class DifferentiableHyperparameterList(nn.Module):
    def __init__(self, hyperparameters, embedding_dim, device):
        ...
        self.hyperparameters = nn.ModuleDict(
            {
                hp: DifferentiableHyperparameter(
    ...
    def get_hyperparameter_info(self):
        ...
        def append_hp(hp_key, hp_val):
            ...
            s_min, s_max, s_mean, s_std = (  # noqa F841
                hp_val.sampler_min,
                hp_val.sampler_max,
                hp_val.sampler_mean,
                hp_val.sampler_std,
            )
            sampled_hyperparameters_f.append(
                (lambda x: (x - s_mean) / s_std, lambda y: (y * s_std) + s_mean)
            )
    ...
    def sample_parameter_object(self):
        ...
        for hp in self.hyperparameters:
            ...
            if isinstance(sampled_hyperparameters_, dict):
                sampled_hyperparameters_ = {
                    hp + "_" + str(key): val
                    for key, val in sampled_hyperparameters_.items()
                }
        ...
        )  # self.pack_parameter_object(sampled_embeddings)


class DifferentiablePrior(torch.nn.Module):
    def __init__(
        self, get_batch, hyperparameters, differentiable_hyperparameters, args
    ):
    ...
    def forward(self):
        ...
        sampled_hyperparameters_passed, sampled_hyperparameters_indicators = (
            self.differentiable_hyperparameters.sample_parameter_object()
        )
...
def get_batch(
    ...
    **kwargs,
    ...
    models = [
        DifferentiablePrior(
            get_batch, hyperparameters, differentiable_hyperparameters, args
        )
    ...
    if len(hyperparameter_matrix[0]) > 0:
        ...
        packed_hyperparameters = torch.repeat_interleave(
            packed_hyperparameters, repeats=batch_size_per_gp_sample, dim=0
        ).detach()
    else:
        packed_hyperparameters = None

    x, y, y_, packed_hyperparameters = (
        torch.cat(x, 1).detach(),
        torch.cat(y, 1).detach(),
        torch.cat(y_, 1).detach(),
        packed_hyperparameters,
    ...
    return (
        ...
        (
            packed_hyperparameters
            if hyperparameters.get("differentiable_hps_as_style", True)
            else None
        ),
    )
...
def merge_style_with_info(diff_hparams_keys, diff_hparams_f, style, transform=True):
    params = dict(
        zip(
            diff_hparams_keys,
            zip(diff_hparams_f, style.detach().cpu().numpy().tolist()[0]),
        )
    )
    ...
    for name, diff_hp_dict in diff_config.items():
        ...
        if distribution == "uniform":
            diff_hp_dict["sample"] = CSH.UniformFloatHyperparameter(
                name, diff_hp_dict["min"], diff_hp_dict["max"]
            )
        elif distribution == "meta_beta":
            diff_hp_dict["k"] = CSH.UniformFloatHyperparameter(
                name + "_k", diff_hp_dict["min"], diff_hp_dict["max"]
            )
            diff_hp_dict["b"] = CSH.UniformFloatHyperparameter(
                name + "_b", diff_hp_dict["min"], diff_hp_dict["max"]
            )
        elif distribution == "meta_gamma":
            diff_hp_dict["alpha"] = CSH.UniformFloatHyperparameter(
                name + "_k", 0.0, math.log(diff_hp_dict["max_alpha"])
            )
            diff_hp_dict["scale"] = CSH.UniformFloatHyperparameter(
                name + "_b", 0.0, diff_hp_dict["max_scale"]
            )
        elif distribution == "meta_choice":
            for i in range(1, len(diff_hp_dict["choice_values"])):
                diff_hp_dict[f"choice_{i}_weight"] = CSH.UniformFloatHyperparameter(
                    name + f"choice_{i}_weight", -3.0, 5.0
                )
            ...
            diff_hp_dict["log_mean"] = CSH.UniformFloatHyperparameter(
                name + "_log_mean",
            ...
            diff_hp_dict["log_std"] = CSH.UniformFloatHyperparameter(
                name + "_log_std", math.log(min_std), math.log(max_std)
            )
        else:
