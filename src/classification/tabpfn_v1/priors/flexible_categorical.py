...
from src.classification.tabpfn_v1.utils import (
    nan_handling_missing_for_a_reason_value,
...
class MulticlassValue(nn.Module):
    def __init__(self, num_classes, ordered_p=0.5):
        super().__init__()
        self.num_classes = class_sampler_f(2, num_classes)()
        self.classes = nn.Parameter(
            torch.randn(self.num_classes - 1), requires_grad=False
        )
        self.ordered_p = ordered_p
...
class MulticlassMultiNode(nn.Module):
    ...
    def forward(self, x):
        ...
        d = torch.multinomial(
            torch.pow(
                0.00001
                + torch.sigmoid(x[:, :, 0 : self.num_classes]).reshape(
                    -1, self.num_classes
                ),
                T,
            ),
            1,
            replacement=True,
        ).reshape(x.shape[0], x.shape[1])  # .float()
        return d
...
class FlexibleCategorical(torch.nn.Module):
        ...
        if self.h["num_classes"] == 0:
            ...
            if self.h["num_classes"] > 1 and not self.h["balanced"]:
                ...
                raise NotImplementedError(
                    "Balanced multiclass training is not possible"
                )
    ...
    def drop_for_no_reason(self, x, v):
        x[
            torch.rand(x.shape, device=self.args["device"])
            < random.random() * self.h["nan_prob_no_reason"]
        ] = v
        return x
    ...
    def forward(self, batch_size):
        ...
        if (
            self.h["nan_prob_no_reason"]
            + self.h["nan_prob_a_reason"]
            + self.h["nan_prob_unknown_reason"]
            > 0
            and random.random() > 0.5
        ):  # Only one out of two datasets should have nans
            if random.random() < self.h["nan_prob_no_reason"]:  # Missing for no reason
                x = self.drop_for_no_reason(
                    x,
                    nan_handling_missing_for_no_reason_value(
            ...
            if (
                self.h["nan_prob_a_reason"] > 0 and random.random() > 0.5
            ):  # Missing for a reason
                x = self.drop_for_reason(
                    x,
                    nan_handling_missing_for_a_reason_value(self.h["set_value_to_nan"]),
                )

            if (
                self.h["nan_prob_unknown_reason"] > 0
            ):  # Missing for unknown reason  and random.random() > 0.5
        ...
        if (
            "categorical_feature_p" in self.h
            and random.random() < self.h["categorical_feature_p"]
        ):
            p = random.random()
            ...
            [
                ...
                torch.zeros(
                    (
                        x.shape[0],
                        x.shape[1],
                        self.args["num_features"] - self.h["num_features_used"],
                    ),
        ...
        if self.h["check_is_compatible"]:
            for b in range(y.shape[1]):
                ...
                while not is_compatible and N < 10:
                    targets_in_train = torch.unique(
                        y[: self.args["single_eval_pos"], b], sorted=True
                    )
                    targets_in_eval = torch.unique(
                        y[self.args["single_eval_pos"] :, b], sorted=True
                    )

                    is_compatible = (
                        len(targets_in_train) == len(targets_in_eval)
                ...
                if not is_compatible:
                    if not is_compatible:
                        ...
                        y[
                            ...
                            -100
                        )  # Relies on CE having `ignore_index` set to -100 (default)
        ...
        if self.h["normalize_labels"]:
            ...
            for b in range(y.shape[1]):
                ...
                if self.h.get("normalize_ignore_label_too", False):
                    valid_labels[:] = True
                y[valid_labels, b] = (
                    (y[valid_labels, b] > y[valid_labels, b].unique().unsqueeze(1))
                    .sum(axis=0)
                    .unsqueeze(0)
                    .float()
                )

                if y[valid_labels, b].numel() != 0 and self.h.get(
                    ...
                    random_shift = torch.randint(
                        0, num_classes, (1,), device=self.args["device"]
                    )
                    y[valid_labels, b] = (
                        y[valid_labels, b] + random_shift
                    ) % num_classes
...
def get_batch(
    ...
    **kwargs,
    ...
    seq_len = (
        hyperparameters["seq_len_used"]()
        if callable(hyperparameters["seq_len_used"])
        else seq_len
    )
    ...
    models = [
        FlexibleCategorical(get_batch, hyperparameters, args).to(device)
