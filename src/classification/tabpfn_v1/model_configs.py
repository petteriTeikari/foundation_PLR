...
from src.classification.tabpfn_v1.priors.utils import uniform_int_sampler_f
from src.classification.tabpfn_v1.priors.differentiable_prior import (
    DifferentiableHyperparameter,
)
...
def get_general_config(max_features, bptt, eval_positions=None):
    ...
    config_general = {
        ...
        "batch_size": CSH.CategoricalHyperparameter(
            "batch_size", [2**i for i in range(6, 8)]
        ),
...
def get_flexible_categorical_config(max_features):
    ...
    config_flexible_categorical = {
        "nan_prob_unknown_reason_reason_prior": CSH.CategoricalHyperparameter(
            "nan_prob_unknown_reason_reason_prior", [0.5]
        ),
        "categorical_feature_p": CSH.CategoricalHyperparameter(
            "categorical_feature_p", [0.0, 0.1, 0.2]
        ),
        "nan_prob_no_reason": CSH.CategoricalHyperparameter(
            "nan_prob_no_reason", [0.0, 0.1]
        ),
        "nan_prob_unknown_reason": CSH.CategoricalHyperparameter(
        ...
        "balanced": True,
        "normalize_to_ranking": CSH.CategoricalHyperparameter(
            "normalize_to_ranking", [False]
        ),
        "set_value_to_nan": CSH.CategoricalHyperparameter(
            "set_value_to_nan", [0.5, 0.2, 0.0]
        ),
...
def sample_differentiable(config):
    ...
    for k, v in config["differentiable_hyperparameters"].items():
        s_indicator, s_hp = DifferentiableHyperparameter(
            **v, embedding_dim=None, device=None
        )()  # both of these are actually not used to the best of my knowledge
        result[k] = s_hp
