...
class TestLoadModuleOnlyInference(unittest.TestCase):
    def test_main(self):
        ...
        for (
            dataset
        ) in test_datasets:  # only test first dataset, can be removed to test all
            xs, ys = dataset[1].clone(), dataset[2].clone()
            eval_position = xs.shape[0] // 2
            train_xs, train_ys = xs[0:eval_position], ys[0:eval_position]
            test_xs, test_ys = xs[eval_position:], ys[eval_position:]  # noqa F841

            classifier = TabPFNClassifier(device="cpu")
            classifier.fit(train_xs, train_ys)
            pred1 = classifier.predict_proba(test_xs)
            self.assertEqual(
                classifier.models_in_memory, TabPFNClassifier.models_in_memory
            )
            ...
            for number in range(number_of_predictions):
                for class_nr in range(number_of_classes):
                    ...
                    self.assertTrue(pred1[number][class_nr] == pred2[number][class_nr])
