...
def old_torch_nanstd(x, axis=0):
    ...
    return torch.sqrt(
        torch.sum(
            torch.where(
                torch.isnan(x), torch.full_like(x, 0), torch.square(mean_broadcast - x)
            ),
            dim=axis,
        )
        / (num - 1)
...
def old_remove_outliers(X, n_sigma=4, normalize_positions=-1):
    ...
    data_mean, data_std = (
        old_torch_nanmean(data, axis=0),
        old_torch_nanstd(data, axis=0),
    )
    ...
    data_mean, data_std = (
        old_torch_nanmean(data_clean, axis=0),
        old_torch_nanstd(data_clean, axis=0),
    )
    cut_off = data_std * n_sigma
    lower, upper = data_mean - cut_off, data_mean + cut_off
        ...
        for dataset in test_datasets:
            ...
            for number in range(number_of_samples):
                for class_nr in range(number_of_classes):
                    if torch.isnan(xs_new[number][class_nr]) and torch.isnan(
                        xs_old[number][class_nr]
                    ):
                        continue
                    if float(xs_new[number][class_nr]) != float(
                        xs_old[number][class_nr]
                    ):
                        print(
                            float(xs_new[number][class_nr])
                            - float(xs_old[number][class_nr])
                        )
                    # checks that every class probability has difference of at most
                    self.assertEqual(
                        float(xs_new[number][class_nr]), float(xs_old[number][class_nr])
                    )
