...
def cls_train_on_this_combo(run_name: str, cls_model_name: str):
...
def cls_is_on_ground_truth(run_name):
...
def get_cls_baseline_models():
    return ["LogisticRegression", "XGBOOST", "CATBOOST", "TabPFN", "TabM"]


def is_experimental_cls_model(cls_model_name):
...
def is_trained_on_embeddings(run_name):
...
def get_dict_array_splits(dict_arrays: dict) -> list:
...
def preprocess_features(
    train_df: pl.DataFrame, val_df: pl.DataFrame, cls_preprocess_cfg: DictConfig
):
    """
...
def logger_remaining_samples(features: dict, samples_in: dict, source):
    data: dict = features[source]["data"]
...
def drop_unlabeled_subjects(
    df: pl.DataFrame, cfg: DictConfig, label_col_name: str = "metadata_class_label"
):
...
def drop_useless_cols(features, cfg):
...
def check_classification_labels(features, source, split, features_in):
    assert (
...
def keep_only_labeled_subjects(features: dict, cfg: DictConfig, data_key: str = "data"):
...
def get_numpy_boolean_index_for_class_labels(label_array: np.ndarray):
...
def index_with_boolean_all_numpys_in_datadict(
    data_dict: dict, labeled_boolean: np.ndarray
):
...
def keep_only_labeled_subjects_from_source(data_dicts: dict, cfg: DictConfig):
...
def check_data_for_NaNs(source, features_per_source):
...
def classifier_hpo_eval(y_true, pred_proba, eval_metric, model: str, hpo_method: str):
