...
def log_classifier_sources_as_params(
    features_per_source: dict[str, Optional[dict[str, Any]]],
    dict_arrays: dict[str, Any],
    run_name: str,
    cfg: DictConfig,
) -> None:
    """
    Log upstream pipeline run IDs and featurization settings to MLflow.

    Logs the run_id of different preprocessing tasks (imputation, featurization,
    outlier detection) so artifacts can be fetched later for summarization.

    Parameters
    ----------
    features_per_source : dict
        Dictionary containing MLflow run info for upstream tasks with keys
        'mlflow_run_imputation', 'mlflow_run_featurization', 'mlflow_run_outlier_detection'.
    dict_arrays : dict
        Dictionary containing 'subject_codes_train' and 'subject_codes_test'.
    run_name : str
        Classifier run name used to determine featurization method.
    cfg : DictConfig
        Hydra configuration with CLASSIFICATION_SETTINGS.DIM_REDUCTION for embeddings.

    Returns
    -------
    None
        Logs parameters directly to active MLflow run.
    """
    keys = [
...
def log_source_metrics_as_params(mlflow_run_feat: Optional[pd.Series]) -> None:
    """
    Log preprocessing metrics from featurization run as MLflow params.

    Parameters
    ----------
    mlflow_run_feat : pd.Series
        MLflow run info from featurization with imputation/outlier metrics.
    """
...
def parse_and_log_cls_run_name(run_name: str, delimiter: str = "__") -> None:
    """
    Parse run name and log preprocessing sources to MLflow.

    Extracts classifier, feature method, imputation source, and anomaly
    source from run name and logs as MLflow parameters.

    Parameters
    ----------
    run_name : str
        Run name like 'CatBoost__ampbins_pipr__SAITS__MOMENT-gt-finetune'.
    delimiter : str, default "__"
        Delimiter separating name components.
    """
    try:
...
def get_artifact_fileinfo(
    best_run: pd.Series, artifact_subdir: str
) -> mlflow.entities.FileInfo:
    """
    Get artifact file info from MLflow run.

    Parameters
    ----------
    best_run : pd.Series
        MLflow run info with run_id.
    artifact_subdir : str
        Subdirectory name to find (e.g., 'model', 'metrics').
...
def import_cls_model_from_mlflow(
    ...
    """
    Load trained classifier model from MLflow artifacts.
    ...
    cfg : DictConfig
        Hydra configuration.
    model_fname : str, default "model.xgb"
        Model filename.
    load_from_autolog : bool, default False
        Load from autolog artifacts (not implemented).
    ...
    NotImplementedError
        If load_from_autolog is True.
    """

    def define_autolog_artifact_path(
        run_id: str, artifact_path: str, model_fname: str
    ) -> str:  # noqa: E501
...
def import_cls_metrics_from_mlflow(
    best_run: pd.Series, cfg: DictConfig
) -> dict[str, Any]:
    """
    Load classifier metrics from MLflow artifacts.
...
def import_cls_dict_arrays_from_mlflow(
    best_run: pd.Series, cfg: DictConfig
) -> dict[str, Any]:
    """
    Load classifier input arrays from MLflow artifacts.
...
def get_previous_best_classifier_run(
    run_name: str, cfg: DictConfig
) -> tuple[bool, Optional[pd.DataFrame]]:
    """
    Search for existing MLflow runs with given name.
    ...
    -------
    tuple
        (found, runs) where found is bool and runs is DataFrame or None.
    """
...
def get_best_cls_run(best_runs: pd.DataFrame, cfg: DictConfig) -> pd.Series:
    """
    Select best run from multiple runs based on configured metric.
    ...
    cfg : DictConfig
        Configuration with BEST_METRIC settings.

    Returns
    -------
    pd.Series
        Best run based on metric and direction.

    Raises
    ------
    ValueError
        If unknown direction or no runs found.
    """

    def get_best_cls_metric_string(cfg: DictConfig) -> str:
...
def import_cls_mlflow_wrapper(
    ...
    """
    Load model, metrics, and data arrays from MLflow for a run name.
    ...
    -------
    tuple
        (model, metrics, dict_arrays) loaded from MLflow.
    """
...
def retrain_classifier(
    ...
    """
    Determine if classifier should be retrained.
    ...
    cfg : DictConfig
        Hydra configuration.
    cfg_key : str, default "CLASSIFICATION_SETTINGS"
        Config key for retrain setting.

    Returns
    -------
    bool
        True if should retrain, False if can use existing.
    """
...
def get_cls_arrays_fname(run_name: str, cfg: DictConfig) -> str:
    """
    Generate filename for classifier input arrays.
    ...
    run_name : str
        Classifier run name.
    cfg : DictConfig
        Hydra configuration (unused).

    Returns
    -------
    str
        Filename like 'dictArrays_{run_name}.pickle'.
    """
    return f"dictArrays_{run_name}.pickle"


def get_input_array_path(run_name: str, cfg: DictConfig) -> str:
    """
    Get full path for classifier input arrays file.
    ...
    -------
    str
        Full file path for input arrays pickle.
    """
    dir_out = Path(get_artifacts_dir(service_name="classification"))
    fname = get_cls_arrays_fname(run_name, cfg)
    return str(dir_out / fname)


def get_cls_metrics_fname(run_name: str, prefix: Optional[str] = None) -> str:
    """
    Generate filename for classifier metrics.
    ...
    -------
    str
        Filename like 'metrics_{run_name}.pickle'.
    """
...
def get_metrics_path(run_name: str, prefix: Optional[str] = None) -> str:
    """
    Get full path for classifier metrics file.
    ...
    -------
    str
        Full file path for metrics pickle.
    """
    dir_out = Path(get_artifacts_dir(service_name="classification"))
...
def get_model_fname(
    run_name: str,
    xgboost_cfg: Optional[DictConfig] = None,
    prefix: Optional[str] = None,
) -> str:
    """
    Generate filename for classifier model.
    ...
    -------
    str
        Filename like 'model_{run_name}.pickle'.
    """
...
def get_model_fpath(
    ...
    """
    Get full path for classifier model file.

    Parameters
    ----------
    run_name : str
        Classifier run name.
    xgboost_cfg : DictConfig, optional
        XGBoost configuration (unused).
    prefix : str, optional
        Prefix to add to filename.
    ...
    -------
    str
        Full file path for model pickle.
    """
    dir_out = Path(get_artifacts_dir(service_name="classification"))
...
def classifier_log_cls_evaluation_to_mlflow(
    ...
    dict_arrays: Optional[dict[str, Any]],
    ...
    """
    Log all classifier evaluation results to MLflow.

    Logs metrics, models, subject stats, uncertainty quantification,
    and input arrays to MLflow artifacts.
    ...
    models : list
        List of bootstrap/ensemble models.
    metrics : dict
    ...
    run_name : str
        MLflow run name.
    model_name : str
        Classifier name for special handling.
    log_manual_model : bool, default True
        Whether to save models as artifacts.
    """
    # TODO! Get this from cfrg, and not as hardcoded
    class_names: dict[int, str] = {0: "Control", 1: "Glaucoma"}

    def log_metrics_stats(metrics: dict[str, Any]) -> None:
    ...
    def log_global_subject_stats(metrics: dict[str, Any]) -> None:
        def log_per_stat_key(
            stats_per_split: dict[str, Any],
            split: str,
            stat_key: str,
            class_key: int,
            class_name: str,
        ...
        global_subject_stats = metrics.get("subject_global_stats")
        if global_subject_stats is None:
            logger.warning("Global subject stats not available, skipping logging")
            return
    ...
    def log_subjectwise_uq(metrics: dict[str, Any]) -> None:
        if metrics.get("subjectwise_stats") is None:
            logger.warning("Subjectwise stats not available, skipping logging")
            return
    ...
    def log_input_arrays(dict_arrays: dict[str, Any]) -> None:
    ...
    def log_metrics_as_pickle(metrics: dict[str, Any]) -> None:
    ...
    def log_baseline_model_as_pickle(
        baseline_results: dict[str, Any], model: Any, cls_model_cfg: DictConfig
    ) -> None:
    ...
    def log_model_manually(
        models: list[Any], run_name: str, cls_model_cfg: DictConfig
    ) -> None:
...
def log_classifier_params_to_mlflow(
    model_params: dict[str, Any], cls_model_cfg: DictConfig
) -> None:
    """
    Log classifier hyperparameters and model config to MLflow.
    ...
    cls_model_cfg : DictConfig
        Classifier model configuration with WEIGHING and DATA settings.
    """
