...
class Euclidean:
    ...
    def measure(self, X, Y, index):
        ...
        if norm == False:  # noqa: E712
            ...
            return score
        elif type(X_train) == int:  # noqa: E721
            ...
            if index + neighbor < self.n_train_ and index - neighbor > 0:
                region = np.concatenate(
                    (
                        data[index - neighbor : index],
                        data[index + window : index + neighbor],
                    )
                )
                ...
                region = np.concatenate(
                    (
                        data[self.n_train_ - neighborhood : index],
                        data[index + window : self.n_train_],
                    )
                )
                D = np.max(region) - np.min(region)
                ...
                region = np.concatenate(
                    (data[0:index], data[index + window : index + neighborhood])
                )
                D = np.max(region) - np.min(region)
    ...
    def set_param(self):
        if self.detector != None:  # noqa: E711
...
class Mahalanobis:
    ...
    def set_param(self):
        ...
        for i in range(number):
            self.residual[:, i] = resid[
                self.n_initial_ + i : self.n_initial_ + i + window
            ]
    ...
    def norm_pdf_multivariate(self, x):
        ...
        if size == len(mu) and (size, size) == sigma.shape:
            ...
            norm_const = 1.0 / (
                math.pow((2 * math.pi), float(size) / 2) * math.pow(det, 1.0 / 2)
            )
            x_mu = np.matrix(x - mu)
    ...
    def measure(self, X, Y, index):
        ...
        cov = self.cov
        if self.probability == False:  # noqa: E712
            if X.shape[0] == mu.shape[0]:
                score = (
                    np.matmul(np.matmul((X - Y - mu).T, cov), (X - Y - mu))
                    / (X.shape[0])
                )
                self.decision_scores_.append((index, score))
                return score
...
class DTW:
    ...
    def __init__(self, method="L2"):
        self.decision_scores_ = []
        if type(method) == str:  # noqa: E721
            if method == "L1":
                distance = lambda x, y: abs(x - y)  # noqa: E731
            elif method == "L2":
                distance = lambda x, y: (x - y) ** 2  # noqa: E731
        else:
            distance = method
...
class EDRS:
    ...
    def set_param(self):
        ...
        # vot = self.vot
        # if vot == False:  # noqa: E712
        #     var = np.var(residual)  # noqa: E841
        # else:
        #     model = arch_model(10 * residual, mean="Constant", vol="garch", p=1, q=1)
        #     model_fit = model.fit(disp="off")
        #     # var = model_fit.conditional_volatility / 10  # noqa: E841

        if self.ep == False:  # noqa: E712
...
class TWED:
    ...
    def measure(self, A, B, start_index):
        ...
        Dlp = lambda x, y: abs(x - y)  # noqa: E731
        ...
        for i in range(1, n):
            for j in range(1, m):
                ...
                C[2] = (
                    ...
                    + Dlp(A[i - 1], B[j - 1])
                    + nu
                    * (abs(timeSA[i] - timeSB[j]) + abs(timeSA[i - 1] - timeSB[j - 1]))
                )
