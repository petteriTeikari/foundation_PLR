...
# import eif as iso
from src.anomaly_detection.TSB_AD.models.base import BaseDetector
import numpy as np
import random as rn
from src.anomaly_detection.TSB_AD.utils.utility import zscore
    ...
    def CheckExtensionLevel(self):
        ...
        if self.exlevel > dim - 1:
            raise Exception(
                "Your data has "
                + str(dim)
                + " dimensions. Extension level can't be higher than "
                + str(dim - 1)
                + "."
            )
    ...
    def compute_paths(self, X_in=None):
        ...
        for i in range(len(X_in)):
            ...
            for j in range(self.ntrees):
                h_temp += (
                    PathFactor(X_in[i], self.Trees[j]).path * 1.0
...
class iTree(object):
    ...
    def __init__(self, X, e, l, exlevel=0):  # noqa E741
        """
        ...
        self.l = l
        self.p = (
            None  # Intercept for the hyperplane for splitting data at a given node.
        )
        self.n = (
            None  # Normal vector for the hyperplane for splitting data at a given node.
        )
        self.exnodes = 0
    ...
    def make_tree(self, X, e, l):  # noqa E741
        """
        make_tree(X,e,l)
        Builds the tree recursively from a given node. Returns a Node object.
        ...
        if (
            e >= l or len(X) <= 1
        ):  # A point is isolated in traning data, or the depth limit has been reached.
            left = None
            right = None
            self.exnodes += 1
