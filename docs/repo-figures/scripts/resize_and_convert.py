...
Usage:
    ...
    python resize_and_convert.py --input fig-repo-01-what-this-repo-does.png  # Single file
"""
...
def main():
    ...
    parser.add_argument(
        "--input", type=str, help="Process single file (filename only, not path)"
    )
    parser.add_argument(
        "--width",
        type=int,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview without saving files"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    print(
        f"{'[DRY RUN] ' if args.dry_run else ''}Processing {len(input_files)} files..."
    )
    ...
    for i, input_path in enumerate(input_files, 1):
        ...
        if output_path.exists() and not args.force and not args.dry_run:
            if is_placeholder(output_path):
                ...
                skipped += 1
                print(
                    f"[{i}/{len(input_files)}] SKIP (already converted): {input_path.name}"
                )
                continue
        else:
        ...
        try:
            ...
            if args.dry_run:
                ...
                print(
                    f"         {result['original_size_mb']:.1f}MB ({orig_dim}) -> {new_dim}"
                )
            else:
